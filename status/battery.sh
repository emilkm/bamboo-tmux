show_battery() {
  ...
  index=$1
  icon=$(get_tmux_option "@bamboo_battery_icon" "#{battery_icon}")
  color=$(get_tmux_option "@bamboo_battery_color" "$thm_yellow")
  text=$(get_tmux_option "@bamboo_battery_text" "#{battery_percentage}")
