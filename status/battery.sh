show_battery() {
  ...
  local index=$1
  local icon=$(get_tmux_option "@bamboo_battery_icon" "#{battery_icon}")
  local color=$(get_tmux_option "@bamboo_battery_color" "$thm_yellow")
  local text=$(get_tmux_option "@bamboo_battery_text" "#{battery_percentage}")
